...
-- 14. TABELAS SOCIAIS (FRIENDSHIPS & DIRECT MESSAGES)
-- ------------------------------------------------------------------------------

-- Tabela de Amizades e Solicitações de Amizade (friendships)
...
-- Trigger de updated_at para a tabela friendships
...
-- Habilitar RLS em friendships
alter table public.friendships enable row level security;

create policy "Usuários podem ver suas próprias conexões de amizade" on public.friendships
  for select using (
    auth.uid() = user_id or auth.uid() = friend_id
  );

create policy "Usuários podem enviar solicitações de amizade" on public.friendships
  for insert with check (
    auth.uid() = user_id
  );

create policy "Usuários envolvidos podem atualizar a relação de amizade" on public.friendships
  for update using (
    auth.uid() = user_id or auth.uid() = friend_id
  );

create policy "Usuários envolvidos podem excluir a amizade" on public.friendships
  for delete using (
    auth.uid() = user_id or auth.uid() = friend_id
  );

-- Tabela de Mensagens Privadas / Direct Messages (direct_messages)
create table if not exists public.direct_messages (
  ...
  content text not null,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  constraint check_dm_different_users check (sender_id <> receiver_id)
);

-- Habilitar RLS em direct_messages
alter table public.direct_messages enable row level security;

create policy "Usuários podem ler mensagens em que são remetentes ou destinatários" on public.direct_messages
  for select using (
    auth.uid() = sender_id or auth.uid() = receiver_id
  );

create policy "Usuários podem enviar mensagens privadas para seus amigos" on public.direct_messages
  for insert with check (
    auth.uid() = sender_id and
    exists (
...
-- Habilitar publicação Realtime para direct_messages e friendships
do $$
begin
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
